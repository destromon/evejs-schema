define(function() {
    ...
    public.template = controller.path('temp/template') + '/form.html';
    ...
    var _setData = function(next) {
		...
		if(post && post.length) {
			//query to hash
			this.data.temp = $.queryToHash(post);
    ...
    var _listen = function(next) {
		$('form.package-temp-form').on('keyup', 'input[name="title"]', function(e) {
			var name = $(this);
	...
	var _process = function(next) {
		var url = controller.getServerUrl() + '/temp/create';
		
		//save data to database
		$.post(url, this.data.temp, function(response) {
			response = JSON.parse(response);
			
			if(!response.error) {		
				controller				
					//display message status
					.notify('Success', 'temp successfully created!', 'success')
					//go to listing
					.redirect('/temp');
